...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            x_l, y_l, z_l, w_l, l_l, yaw_l = label.box.center_x, label.box.center_y, label.box.center_z, label.box.width, label.box.length, label.box.heading
            corner_label = tools.compute_box_corners(x_l, y_l, w_l, l_l, yaw_l)
            ## step 2 : loop over all detected objects      
            for obj in detections:
                ## step 3 : extract the four corners of the current detection
                _, x_o, y_o, z_o, h_l, w_o, l_o, yaw_o  = obj
                obj_label = tools.compute_box_corners(x_o, y_o, w_o, l_o, yaw_o) 
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dist_x = np.float_(x_l) - np.float_(x_o)
                dist_y = np.float_(y_l) - np.float_(y_o)
                dist_z = np.float_(z_l) - np.float_(z_o)
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                a = Polygon(corner_label)
                b = Polygon(obj_label)
    ...
    #print("len(detections): ", len(detections), ", len(ious): ", len(ious), "true_positives: ", true_positives)
    # compute positives and negatives for precision/recall
    
    ## step 1 : compute the total number of positives present in the scene
    all_positives = np.count_nonzero(labels_valid)
...
def compute_performance_stats(det_performance_all):
    ...
    _, tp, fn, fp = [sum(x) for x in zip(*pos_negs)]
